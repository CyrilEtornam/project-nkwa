...
## 22. Presentation Slide Deck (Outline)

Prepared for the project presentation. Slides 2, 4, and 7 are the strongest beats: lead emotionally with the problem, prove it with the live demo, impress with the language-routing engineering. Keep on-screen text minimal — the detail below is speaker notes.

1. **Title** — Nkwa (Twi for "life"), Ghana 112 Emergency Copilot. "Call for help in your own language." AWS GenAI Hackathon (AmaliTech × AWS re:Start). Presenter names + date.
2. **The Problem** — 90% of 112 calls are pranks (Akufo-Addo, Jan 2020); GNFS 131,862 prank calls Q1 2025; 16.9-min avg ambulance response vs 8-min target; 1 ambulance per 250,000 people in Accra; no street addressing; 80+ languages; callers get zero guidance. Land line: "Overwhelmed, can't be located, can't be understood."
3. **The Solution** — GenAI dispatch copilot, end-to-end in under 30s: listen in any language → transcribe + translate → classify severity (CRITICAL/URGENT/NON_EMERGENCY) → detect pranks → GPS to landmark → first-aid audio in caller's language → complete dispatcher brief before pickup.
4. **Live Demo** (anchor) — caller picks service → language → records → Stop & Send → result: triage + landmark + first-aid audio; dispatcher brief with severity colour-coding. Fallback: pre-cached simulator scenario if live audio/network fails.
5. **Architecture** — one FastAPI backend, three frontends (Flutter caller, React web caller, React dispatcher), AWS-native (EC2, DynamoDB, S3, WebSocket). Left-to-right diagram: Caller → API → AI pipeline → Dispatcher.
6. **The Pipeline** — POST /calls/initiate: Audio → ASR → Translate → Bedrock triage (severity + prank + first-aid script + brief in one call) → Location (GPS→landmark) → TTS first-aid audio → dispatcher brief. < 30s, one structured response.
7. **Smart Language Routing** (standout) — local languages → Khaya AI/GhanaNLP; English → Amazon Transcribe + Polly (zero Khaya credits). Routed once in speech_client.py with DynamoDB caching to respect Khaya's 100-req/month limit. Cost-aware, scalable, built around a real API constraint.
8. **AI Triage with Bedrock** — Claude 3.5 Sonnet, single inference returns severity, prank flag, incident type, first-aid script (English + local), dispatcher brief. WHO guidelines embedded in prompt (no RAG for MVP).
9. **Impact** — filters pranks, locates the unlocatable, breaks the language barrier, coaches first aid during the wait, brief ready before the dispatcher answers.
10. **Tech Stack** — Python 3.11, FastAPI, Bedrock (Claude), Khaya AI, Transcribe/Polly, AWS Location, DynamoDB, S3, SNS, React, Flutter, EC2.
11. **Roadmap** — re-enable JWT auth, registration + OTP, emergency contacts, SOS panic button, SNS SMS alerts, live dispatcher WebSocket feed; streaming ASR; more languages; pilot with real 112 dispatchers.
12. **Close** — "Nkwa — life. Anyone in Ghana can call for help, in their own language, and get guided care in under 30 seconds." Thank you + Q&A.
